<template>
  <div class="email-display">
    ...
    <div>
      <em>From {{ email.from }} on {{ format(new Date(email.sentAt), 'MMM do yyy') }}</em>
    </div>
...
<script>
  ...
  export default {
    setup(props, {emit}) {
      ...
      let goOlrderAndArchive = () => { emit('changeEmail', { changeIndex: 1, toggleArchive: true, save: true }) }
